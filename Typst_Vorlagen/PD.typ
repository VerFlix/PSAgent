...
#set text(12pt)
= PSA-Produktdatenblatt (PD) SachkundigenPrüfung
...
#grid(
  ...
  [
    #box(stroke: 0.8pt, inset: 6pt)[
      #set text(9pt)
  *Dieses Produktdatenblatt ist dem beschriebenen Produkt zugeordnet und ist bei
  der nächsten PSA-Überprüfung zusammen mit dem Produkt einzureichen
  (Historie der Überprüfung). Das Produkdatenblatt (PD) und Gebrauchsanleitung (GAL)
  sind so aufzubewahren, dass sie eindeutig dem Produkt zu zuordnen sind.* \
  Der PSA-Sachkundige übernimmt keine Haftung für personelle oder materielle Folgeschäden aufgrund von nicht sichtbaren Schäden an textilen Elementen durch Kontakt mit Chemikalien, Säuren und Basen oder anderen schädlichen Substanzen.
]
...
#let check-block() = [
  ...
  #box(stroke: 0.8pt, inset: 8pt)[
    *Überprüfung*
    ...
    #grid(
      columns: (0.3fr, 1fr, 1fr, 0.4fr),
      gutter: 6pt,
      [
        Datum: \ #v(8pt) #line() \
        Nächste Prüfung: \ #v(8pt) #line()
      ],[
        Grund der Überprüfung: \ #line()
        \ #v(8pt) #line()
        \ #v(8pt) #line()
        \ #v(8pt) #line()
      ],[
        [ ] Einwandfrei und einsatzfähig \
        [ ] Kommentare, Reparaturen, Beobachtungen:
        #v(8pt)
        #line()
        #v(8pt)
        #line()
        [ ] Ausgesondert, nicht freigegeben; Grund:
        \ #v(8pt) #line()
      ],[
